...
broad_dir="$(mktemp -d "${TMPDIR:-/tmp}/infra-kit-broad.XXXXXX")"
touch "$broad_dir/sentinel"
if bash "$KIT_ROOT/bootstrap/sync.sh" --version test \
  --vendor-dir "$broad_dir" --source "$KIT_ROOT" >/dev/null 2>&1; then
  fail "sync accepted a vendor directory not ending in infra-kit"
fi
[[ -f "$broad_dir/sentinel" ]] || fail "sync changed a rejected vendor directory"
rm -rf -- "$broad_dir"
pass "sync rejects a broad vendor directory"
