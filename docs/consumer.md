...
Nothing is installed yet, so fetch `sync.sh` once by hand. The repository is
private, so this needs an authenticated `gh` — a plain `curl` of a raw file
returns 404:

```bash
gh api repos/fridthjof-labs/infra-kit/contents/bootstrap/sync.sh?ref=v0.1.0 \
